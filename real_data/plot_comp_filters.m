function ax= plot_comp_filters(pos, nrows, ncols, data, beta_fp_cos, beta_fp_mord, beta_enkf_cos, beta_enkf_mord, curve, city)
...
legend_ekf_cos = ['EnKF - MSEI_1 ', sprintf('(RMSE = %.2f)', rmse(casos,beta_enkf_cos(:,ref+i) ))];
legend_fp_cos = ['FP - MSEI_1 ', sprintf('(RMSE = %.2f)', rmse(casos,beta_fp_cos(:,ref+i)))]; 
legend_ekf_mord = ['EnKF - MSEI_2 ', sprintf('(RMSE = %.2f)', rmse(casos,beta_enkf_mord(:,ref+i)))];
legend_fp_mord = ['FP - MSEI_2 ', sprintf('(RMSE = %.2f)', rmse(casos,beta_fp_mord(1:52,ref+i)))]; 
